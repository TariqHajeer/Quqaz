...
export class CreateMultiOrderAgentAndClientComponent implements OnInit {
  ...
  addNewCountry() {
    ...
    if (!find) {
      this.customerService
        .Create(this.cityapi, this.Order.Country)
        .subscribe((res) => {
  ...
  editNewCountry() {
    if (!this.cities.find((c) => c.name == this.EditOrder.Country.name)) {
      this.customerService
        .Create(this.cityapi, this.EditOrder.Country)
        .subscribe((res) => {
  ...
  Getcities() {
    this.customerService.getAll(this.cityapi).subscribe((res) => {
      ...
      if (this.CountryId) {
        ...
        this.Order.DeliveryCost = this.cities.find(
  ...
  changeCountry() {
    ...
    if (
      ...
      this.Agents = this.GetAgents.filter(
        (a) =>
          a.countries.map((c) => c.id).filter((co) => co == this.CountryId)
            .length > 0
      );
    ...
    this.orderservice
      ...
      .subscribe((res) => {
        for (let i = 0; i < res.length; i++) {
          ...
          if (
            this.Orders.find(
              (o) => o.Code == res[i].code && res[i].avilabe == false
            )
          ) {
  ...
  CheckCodeForChange(code) {
    ...
    this.orderservice.chekcCode(code, this.ClientId).subscribe((res) => {
      if (
        res ||
        this.Orders.filter((o) => o.Code == code && this.ClientId == o.ClientId)
          .length > 0
      ) {
        this.showMessageCodeChange = true;
        return;
      } else this.showMessageCodeChange = false;
    });
  ...
  CheckCode() {
    ...
    if (this.Order.Code != null && this.Order.Code != undefined) {
      this.orderservice
        .chekcCode(this.Order.Code, this.ClientId)
        .subscribe((res) => {
          if (
            res ||
            this.Orders.filter(
              (o) => o.Code == this.Order.Code && this.ClientId == o.ClientId
            ).length > 0
          ) {
  ...
  CheckCodeForEdit() {
    ...
    if (this.EditOrder.Code != null) {
      this.orderservice
        .chekcCode(this.EditOrder.Code, this.ClientId)
        .subscribe((res) => {
          if (this.EditOrder.CanEdit == true)
            if (
              res ||
              this.Orders.filter(
                (o) => o.Code == this.EditOrder.Code && o != this.tempcode
              ).length > 0
            ) {
              this.showEditMessageCode = true;
            } else this.showEditMessageCode = false;
        });
    }
  }
  ...
  Save(order: CreateMultipleOrder) {
    ...
    if (this.checkLengthPhoneNumberEdit(this.EditOrder.RecipientPhones)) return;
