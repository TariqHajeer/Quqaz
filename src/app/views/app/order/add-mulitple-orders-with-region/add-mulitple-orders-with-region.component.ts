...
export class AddMulitpleOrdersWithRegionComponent implements OnInit {
  ...
  getAgent() {
    this.userService.ActiveAgent().subscribe(res => {
      ...
      if (agent) {
        ...
        this.regions=this.cities.find(c=>c.id==this.CountryId).regions
        var region= JSON.parse(localStorage.getItem('regionid'))
        if(region)
  ...
  changeCountryId() {
    ...
    if (this.Orders.length > 0)
      this.Orders.forEach(o => {
        o.DeliveryCost = city.deliveryCost
  ...
  changed(index) {
    if (index == 5) { this.onEnter(); return }
