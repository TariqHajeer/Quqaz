...
export class AddUserComponent implements OnInit, OnChanges {
  ...
  checkName() {
    if (
      ...
      return;
    } else this.nameIsRepeated = false;
  }
  checkUserName() {
    if (
      this.UserService.users.filter(
        (c) => c.userName == this.CreateUser.UserName
      ).length > 0
    ) {
  ...
  addOrEditUser() {   
    ...
    if (this.CreateUser.CanWorkAsAgent) {
      if (
        ...
        this.CreateUser.Countries.length == 0 ||
        !this.CreateUser.Salary ||
      ...
      if (
        ...
        this.CreateUser.GroupsId.length == 0 ||
