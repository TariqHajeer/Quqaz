...
@Component({
  selector: 'app-client-inside-company',
  templateUrl: './client-inside-company.component.html',
  styleUrls: ['./client-inside-company.component.scss']
})
export class ClientInsideCompanyComponent implements OnInit {
  ...
  GetMoenyPlaced() {
    this.orderservice.MoenyPlaced().subscribe(res => {
      ...
      this.MoenyPlaced = this.MoenyPlaced.filter(m => m.id == 1 || m.id == 3)
      this.monePlacedId = 3

    })
  }
  ChangeClientIdOrmonePlacedId() {
    if (this.ClientId != null) {
      this.filtering.MonePlacedId = this.monePlacedId
