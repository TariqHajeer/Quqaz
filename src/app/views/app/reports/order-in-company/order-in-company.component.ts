...
export class OrderInCompanyComponent implements OnInit {
  ...
  addOrder() {
    ...
    if (this.Code) {
      this.orderservice.OrderInCompany(this.ClientId, this.Code).subscribe(
        (res) => {
          this.findorder = res;
          if (this.findorder) {
            this.addOrders();
  ...
  addOrders() {
    ...
    if (
      this.getorder.order.orderplaced.id == OrderplacedEnum.PartialReturned ||
      this.getorder.order.orderplaced.id == OrderplacedEnum.Delivered ||
      this.getorder.order.orderplaced.id == OrderplacedEnum.Way
    )
      this.getorder.canEditCount = false;
    else this.getorder.canEditCount = true;
