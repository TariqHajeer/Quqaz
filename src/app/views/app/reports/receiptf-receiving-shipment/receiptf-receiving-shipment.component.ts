...
GetorderPlace() {
  this.orderservice.orderPlace().subscribe(res => {
    this.orderPlace = res
    this.orderPlace = this.orderPlace.filter(o => o.id ==OrderplacedEnum.PartialReturned || o.id ==OrderplacedEnum.Delivered)
  })
}
changeOrderPlaced() {
  if (this.getorders.length != 0) {
...
addOrder() {
  ...
  if (this.Code) {
    this.orderservice.GetOrderToReciveForDelivredOrders(this.Code).subscribe(res => {
